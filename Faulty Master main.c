...
// UART0 REMAPPED: P0.6 (TXD) and P0.7 (RXD) - RS485 connected to these alternative pins
// This frees P3.0 for fnd13 display use without conflict
...
sbit fnd11 = P2^2;
sbit fnd12 = P3^3;
sbit fnd13 = P3^0; // P3.0 is FREE for display (UART remapped to P0.6/P0.7)
...
/* Feature 4: Backend values and Sigma values */
xdata unsigned long backend_total_val = 0;   // Calculated Total Flow (from regs 5-6)
xdata unsigned long backend_fr_val = 0;      // Calculated Flow Rate (from regs 0-1)
xdata unsigned long sigma_plus_val = 0;      // Sigma + (from regs 8-9)
xdata unsigned long sigma_minus_val = 0;     // Sigma - (from regs 11-12)
...
/* Feature 1: DP blink on data reception */
xdata volatile unsigned int dp_blink_counter = 0;

/* Feature 2: Connection status tracking */
xdata volatile unsigned long last_successful_parse_ms = 0;
volatile bit is_slave_connected = 0;
#define CONNECTION_TIMEOUT_MS  10000  // 10 seconds without response = disconnected (increased for stability)
#define CONNECTION_FAIL_THRESHOLD  3  // Require 3 consecutive failures before marking disconnected (hysteresis)
...
#define MODBUS_TIMEOUT_MS  2000  // 2 second timeout (increased for slowest slaves and bus delays)
#define MODBUS_MAX_RETRIES 3

/* Retry and error tracking */
data unsigned char modbus_retry_count = 0;
data unsigned char consecutive_failures = 0;  // Track consecutive failures for hysteresis
...
void Timer0_ISR(void) interrupt 1
{
  ...
  // Feature 2: Check if slave is connected, if not show dashes
  if (!is_slave_connected) {
    // Display dashes on Total and Flow Rate displays
    switch (scan_d)
    {
      ...
      case 0: case 1: case 2: case 3: case 4:
        display_dash();
        if (scan_d == 0) fnd13=1;
        else if (scan_d == 1) fnd12=1;
        else if (scan_d == 2) fnd11=1;
        else if (scan_d == 3) fnd10=1;
        else if (scan_d == 4) fnd4=1;
        break;
      ...
      case 5: case 6: case 7: case 8: case 9:
        display_dash();
        if (scan_d == 5) fnd9=1;
        else if (scan_d == 6) fnd8=1;
        else if (scan_d == 7) fnd7=1;
        else if (scan_d == 8) fnd6=1;
        else if (scan_d == 9) fnd5=1;
        break;
        
      // --- PET (ID) - Show normal ID ---
      case 10: 
  ...
  // Feature 1: DP blink - Control H_Segment based on blink counter
  // Blink pattern: 100ms on, 100ms off (with 2ms ISR, ~50 calls = 100ms)
  if (dp_blink_counter > 0) {
    // Blink pattern: toggle every 50 ISR calls (100ms)
    if ((dp_blink_counter / 50) & 0x01) {
      H_Segment = 0;  // DP ON (active low)
    }
  }
...
void Timer2_ISR (void) interrupt 5
{
  ...
  // Feature 1: Decrement DP blink counter
  if (dp_blink_counter > 0) dp_blink_counter--;
...
void uart0_init_9600_timer3(void)
{
  ...
  // 2. CRITICAL: Remap UART0 to P0.6 (TXD) and P0.7 (RXD)
  // Hardware RS485 transceiver is connected to P0.6/P0.7, NOT default P3.0/P3.1
  // This frees P3.0 for fnd13 display use without conflict!
  AUXR1 |= 0x80;  // Set bit 7: Remap UART0 to alternative pins
  ...
  // Configure P0.6 and P0.7 for UART (alternative pins)
  // P0.6 = TXD (push-pull mode for output)
  // P0.7 = RXD (input mode for receive)
  P06_PUSHPULL_MODE;   // TXD must be push-pull
  P07_INPUT_MODE;      // RXD must be input
  
  // P3.0 is now FREE for fnd13 display since UART remapped to P0.6/P0.7
  P30_PUSHPULL_MODE;   // Configure P3.0 for fnd13 display control
...
static bit modbus_parse_response(void)
{
  ...
  unsigned long fr_val, total_val, sigma_plus, sigma_minus;
  unsigned char actual_slave_id;
  
  if (modbus_rx_len < 33) return 0;
  
  // Feature 3: Extract actual slave ID from response
  actual_slave_id = modbus_rx_buf[0];
  
  // Validate response (check function code, not slave ID since we want to capture actual ID)
  ...
  // Parse Instantaneous Flow Rate (Address 00H, regs 0-1, bytes 3-6)
  ...
  // Parse Total Flow (Address 05H, regs 5-6, bytes 13-16)
  ...
  // Feature 4: Parse Sigma + (Address 08H, regs 8-9, bytes 19-22)
  high_word = ((unsigned int)modbus_rx_buf[19] << 8) | modbus_rx_buf[20];
  low_word = ((unsigned int)modbus_rx_buf[21] << 8) | modbus_rx_buf[22];
  sigma_plus = ((unsigned long)high_word << 16) | low_word;
  
  // Feature 4: Parse Sigma - (Address 0BH, regs 11-12, bytes 25-28)
  high_word = ((unsigned int)modbus_rx_buf[25] << 8) | modbus_rx_buf[26];
  low_word = ((unsigned int)modbus_rx_buf[27] << 8) | modbus_rx_buf[28];
  sigma_minus = ((unsigned long)high_word << 16) | low_word;
  
  // Feature 3: Update display ID with actual slave ID
  if (actual_slave_id >= 1 && actual_slave_id <= 247) {
    disp_id_u = actual_slave_id;
  }
  
  // Feature 4: Store backend values
  backend_total_val = total_val % 100000UL;
  if (fr_val > 99999UL) fr_val = 99999UL;
  backend_fr_val = fr_val;
  
  // Feature 4: Set display values to Sigma values
  sigma_plus_val = sigma_plus % 100000UL;
  sigma_minus_val = sigma_minus % 100000UL;
  
  // By default, display Sigma values
  disp_total_u = sigma_plus_val;    // Display Sigma + on Total
  disp_fr_u = sigma_minus_val;      // Display Sigma - on Flow Rate
  
  // Feature 1: Trigger DP blink (500ms = 500 ISR ticks at 1ms)
  dp_blink_counter = 500;
  
  // Feature 2: Update last successful parse timestamp
  last_successful_parse_ms = ms_ticks;
...
void modbus_rx_task(void)
{
  // Frame detection: 10ms gap indicates end of frame (increased for reliability)
  // At 9600 baud, 3.5 char times ~= 3.6ms, using 10ms for safety margin
  if (modbus_rx_len > 0 && !modbus_frame_ready) {
    if ((ms_ticks - last_rx_ms) >= 10) modbus_frame_ready = 1;
  }
  ...
  if (modbus_parse_response()) {
    ...
    consecutive_failures = 0;  // Reset hysteresis counter on success
  } else {
...
void main(void)
{
  ...
  while (1)
  {
    ...
    // Feature 2: Check connection timeout with hysteresis
    // Only mark disconnected after multiple consecutive failures to prevent false alarms
    if (is_slave_connected) {
      if ((ms_ticks - last_successful_parse_ms) >= CONNECTION_TIMEOUT_MS) {
        if (consecutive_failures >= CONNECTION_FAIL_THRESHOLD) {
          is_slave_connected = 0;  // Mark as disconnected after threshold reached
        }
      }
    }
    
    // Feature 4: Button handling for display mode switching
    // NOTE: Only update display when button state changes to avoid constant updates
    // The default Sigma values are already set in modbus_parse_response()
    {
      static bit last_sw_set = 1;
      static bit last_sw_reset = 1;
      
      // Detect button state changes
      if (sw_set != last_sw_set || sw_reset != last_sw_reset) {
        if (sw_set == 0) {
          // sw_set pressed: Show calculated Total Flow on Total display
          disp_total_u = backend_total_val;
          update_display_digits();
        } else if (sw_reset == 0) {
          // sw_reset pressed: Show backend flow rate value on Flow display
          disp_fr_u = backend_fr_val;
          update_display_digits();
        } else {
          // Buttons released: Restore Sigma values (if connected)
          if (is_slave_connected) {
            disp_total_u = sigma_plus_val;   // Sigma + on Total
            disp_fr_u = sigma_minus_val;     // Sigma - on Flow Rate
            update_display_digits();
          }
        }
        last_sw_set = sw_set;
        last_sw_reset = sw_reset;
      }
    }
    ...
    if (waiting_for_response) {
      if ((ms_ticks - request_sent_ms) >= MODBUS_TIMEOUT_MS) {
        // Timeout occurred
        modbus_error_count++;
        consecutive_failures++;  // Increment failure counter on timeout
        ...
        if (modbus_retry_count < MODBUS_MAX_RETRIES) {
          ...
          waiting_for_response = 0;
          modbus_retry_count = 0;
          last_poll_ms = ms_ticks;  // Update poll time to enforce POLL_INTERVAL_MS delay before next attempt
        }
      }
    }
...
/* Feature 2: Display dash character for disconnection indication */
void display_dash(void)
{
  // Dash is just the middle segment (G)
  A_Segment=1; B_Segment=1; C_Segment=1; D_Segment=1; E_Segment=1; F_Segment=1; G_Segment=0;
}